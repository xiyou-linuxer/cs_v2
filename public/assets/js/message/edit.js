...
/***/ function(module, exports, __webpack_require__) {
	...
	var $messageEditPage = $('.koala-message-edit-page');
	...
	$messageEditPage.find('#message_form').formValidation({
	  ...
	  if (!data.content) {
	    ...
	    return _ui2.default.alert('内容不能为空~');
	  }
	  ...
	  if (messageId) {
	    _message2.default.update(messageId, data).then(function (res) {
	      resolve(res);
	    }).catch(function (err) {
	      reject(err);
	    });
	  } else {
	    _message2.default.create(data).then(function (res) {
	      resolve(res);
	    }).catch(function (err) {
	      reject(err);
	    });
