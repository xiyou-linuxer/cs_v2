...
/***/ function(module, exports, __webpack_require__) {
	...
	var $profilePage = $('.koala-profile-page');
	var $profileForm = $profilePage.find('#profile_form');
	...
	$profileForm.formValidation({
	  ...
	  var userId = $profileForm.find('#user_id').val();
	  var data = $('form').serializeObject();

	  _user2.default.update(userId, data).then(function (res) {
	    if (res && res.id) {
	      _ui2.default.alert({
	        message: '信息更新成功~'
	      }).then(function () {
	      ...
	      _ui2.default.alert('信息保存失败，请稍后再试~');
	...
	$profilePage.find('#btn_reset_password').on('click', function (e) {
	  ...
	  _ui2.default.confirm('目前只能通过邮件找回密码， 继续操作之前需要先退出系统。<br>是否继续？').then(function () {
...
/***/ function(module, exports, __webpack_require__) {
	...
	$newsList.delegate('.btn-news-delete', 'click', function () {
	  ...
	  _ui2.default.confirm('确定要删除该动态吗？').then(function () {
	    _news2.default.delete(newsId).then(function () {
	      _ui2.default.alert('动态删除成功~').then(function () {
	        $newsItem.remove();
	      });
	    }).catch(function (err) {
	      if (err && err.message) {
	        ...
	        _ui2.default.alert('动态删除失败，请稍后再试~');
	      }
	    });
	  });
	});
	...
	$newsList.delegate('.btn-news-favor', 'click', function () {
	  ...
	  _news2.default.favor(newsId).then(function (data) {
	    replaceCommentItem(data, $newsItem);
	  }).catch(function (err) {
	    if (err && err.message) {
	      _ui2.default.alert(err.message);
	    } else {
	      _ui2.default.alert('点赞失败，请稍后再试~');
	    }
	  });
	});
	...
	$newsList.delegate('.btn-news-comment', 'click', function () {
	  ...
	  if (!content) {
	    _ui2.default.alert('既然来了，不如说点什么吧');
	    return false;
	  }

	  _news2.default.createComment(newsId, { content: content }).then(function (data) {
	...
	function replaceCommentItem(data, $oldItem) {
	  var html = new _xtemplate2.default(newsTemplate).render([data]);
