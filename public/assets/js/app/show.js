...
/***/ function(module, exports, __webpack_require__) {
	...
	var $appShowPage = $('.koala-app-show-page');

	var appId = $appShowPage.find('#data_app_id').val();
	...
	$appShowPage.find('#btn_refresh_secret').on('click', function (e) {
	  _app2.default.refreshSecret(appId).then(function (res) {
	    if (res && res.id) {
	      $appShowPage.find('#app_secrect_field').html(res.secret);
	    } else {
	      _ui2.default.alert('应用secret刷新失败，请稍后再试~');
	    }
	  }).catch(function (err) {
	    if (err && err.message) {
	...
	$appShowPage.find('#btn_app_confirm').on('click', function (e) {
	  _app2.default.confirm(appId).then(function (res) {
	    if (res && res.id) {
	      _ui2.default.alert('应用已审核通过~').then(function () {
	        location.reload();
	      });
	    } else {
	...
	$appShowPage.find('#btn_app_reject').on('click', function (e) {
	  _app2.default.reject(appId).then(function (res) {
	    if (res && res.id) {
	      _ui2.default.alert('操作成功，应用审核不通过~').then(function () {
	        location.reload();
	      });
	    } else {
	      _ui2.default.alert('操作失败，请稍后再试~');
