...
/***/ function(module, exports, __webpack_require__) {
	...
	var $appEditPage = $('.koala-app-edit-page');
	...
	$appEditPage.find('#app_form').formValidation({
	  ...
	  var appId = $appEditPage.find('#data_app_id').val();
	  if (appId) {
	    _app2.default.update(appId, data).then(function (res) {
	      if (res && res.id) {
	        _ui2.default.alert({
	          message: '应用保存成功~'
	        }).then(function () {
	        ...
	        _ui2.default.alert('应用保存失败，请稍后再试~');
	        fv.disableSubmitButtons(false);
	      }
	    }).catch(function (err) {
	      if (err && err.message) {
	    ...
	    _app2.default.create(data).then(function (res) {
	      if (res && res.id) {
	        _ui2.default.alert({
	          message: '应用创建成功~'
	        }).then(function () {
	        ...
	        _ui2.default.alert('应用创建失败，请稍后再试~');
	      }
	    }).catch(function (err) {
	      if (err && err.message) {
	        _ui2.default.alert(err.message);
