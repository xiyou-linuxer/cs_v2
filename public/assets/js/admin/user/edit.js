...
/***/ function(module, exports, __webpack_require__) {
	...
	var $adminUserEditPage = $('.koala-admin-user-edit-page');
	...
	$adminUserEditPage.find('form').formValidation({
	  ...
	  var data = $adminUserEditPage.find('form').serializeObject();

	  var userId = $adminUserEditPage.find('#data_user_id').val();
	  ...
	  if (userId) {
	    _user2.default.update(userId, data).then(function (res) {
	      if (res && res.id) {
	        ...
	        _ui2.default.alert('用户信息更新失败，请稍后再试~');
	    ...
	    _user2.default.create(data).then(function (res) {
	      if (res && res.id) {
	        _ui2.default.confirm({
	          message: '用户信息添加成功，是否继续添加？'
	        }).catch(function () {
	          location.href = '/admin/users';
	        });
	      } else {
	        _ui2.default.alert('用户信息添加失败，请稍后再试~');
	        fv.disableSubmitButtons(false);
	      }
	    }).catch(function (err) {
	      if (err && err.message) {
